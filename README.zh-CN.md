...
0.1.0 版本是一个早期公开版本：自动化隐私/构建门禁、永久签名、逐构件精确校验以及范围受限的真机冒烟测试均已通过。它尚未获得广泛的 API 26–36 设备认证，也未通过人工无障碍/性能认证。v0.2.0 开发线新增如实呈现的、带条件的 5 秒请求模式；在发布前，其实现和真机验证必须通过单独的 [v0.2.0 检查清单](docs/release/v0.2.0-checklist.md)。保障政策仍由 [ADR 0003](docs/adr/0003-early-public-release-policy.md) 定义。
