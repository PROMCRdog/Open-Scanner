...
0.2.0 是当前已发布的早期版本。其带条件的 5 秒请求模式、语言选择器、永久签名、逐构件精确校验以及范围受限的 API 36 真机检查均已通过 [v0.2.0 检查清单](docs/release/v0.2.0-checklist.md)。v0.2.1 开发线移除了频谱图最多显示四个网络的限制，并加入由用户控制的曲线选择、单选焦点以及各网络选择器中的当前 Wi-Fi 标识；在单独的 [v0.2.1 检查清单](docs/release/v0.2.1-checklist.md) 完成前，它仍是候选版本。保障政策仍由 [ADR 0003](docs/adr/0003-early-public-release-policy.md) 定义。
...
- 可选请求间隔为 5、10、15、30 和 60 秒。**5 秒请求模式**仅在前台运行，并且只在 Android 明确报告扫描节流已关闭时可用；界面会显示真实数据来源时间，绝不会把缓存数据变成新的采样点。
- 选中 AP 追踪器：按时间戳缩放、上限 60 个采样点、仅存在于内存的信号历史，并在最新证据不再是最新时显示可见的间隙。
- 基于近期快照的稳定性指标，直接报告 RSSI 波动范围和观测到的缺失占比，而不是要求用户从图表中自行推断信号抖动。
- 独立的 Canvas 信道占用图默认显示所选信道组内的全部 AP，并提供多选显示过滤器、单选焦点控制、相互独立的青色焦点与绿色“当前 Wi-Fi”角色、绑定频宽的主信道标记以及无障碍等效文字描述；未知信道频宽保持明确显示为未知，而不会虚构出 20 MHz 的占用。
...
- 应用内可选择系统默认、English 或简体中文；系统默认会继续跟随设备语言的后续变化，也是全新安装与重置后的默认选项。
...
0.2 版本仍完全采用被动方式。它不加入网络、不收集密码、不探测局域网设备、不做测速、不扫描端口，也不联系任何互联网端点。加入网络和受保护设置由 Android 系统负责。更快的请求不会新增后台服务、权限或 `INTERNET` 访问，并会在应用不再位于前台时停止。
...
- Android 可能节流、拒绝、延迟或复用 Wi-Fi 扫描结果。即使在开发者选项中关闭扫描节流，**5 秒**也只是请求节奏，并不保证每五秒产生一次新的射频测量，而且可能增加耗电。
- 如果 Android 之后报告扫描节流已开启或其状态不可读取，5 秒模式会自动恢复为默认的 30 秒。0.2.0 版本不提供 1 秒模式，也不进行后台扫描。
...
- PNG 导出、加密的保存会话历史/对比、别名/收藏、快照差异对比、演示模式、6 GHz PSC 高亮、主动 DNS/HTTP 测试、局域网发现以及吞吐量测试仍为未来工作。
